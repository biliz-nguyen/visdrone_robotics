#!/usr/bin/env python3
"""Run C3 quality-alignment confirmation on the frozen N2b architecture.

This reuses the exact previously positive TAQ-v1 schedule (16 px, beta floor 4)
without retuning it for N2b. Only training-time TAL quality is changed.
"""
...
def architecture_sanity(report_dir: Path):
    ...
    assert criterion.assigner.__class__.__name__ == "TinyAdaptiveQualityAssigner"
...
def main():
    args = parse_args()
    if args.epochs != 5:
        raise ValueError("C3 TAQ-N2b screening is locked to exactly 5 epochs")
    ...
    cfg["c3_assigner_mode"] = "tiny_quality"
    cfg["tiny_quality"] = {"tiny_min_side": 16.0, "beta_floor": 4.0}
    cfg["train"]["epochs"] = 5
    ...
    summary = {
        "purpose": "C3 TAQ-on-N2b local 5e confirmation; not final paper evidence.",
        "mechanism": {
            "name": "Tiny-Adaptive Quality Alignment (TAQ) on frozen N2b",
            "changed_scope": "TAL localization-quality exponent only",
            "tiny_min_side": 16.0,
            "beta_floor": 4.0,
            "retuned_for_n2b": False,
    ...
    try:
        ...
        eval_path = report_dir / "c3_taq_n2b_v1_eval.json"
        ...
        summary["caution"] = "single local seed, deterministic=false, 5 epochs only; a 50e run requires explicit approval."
