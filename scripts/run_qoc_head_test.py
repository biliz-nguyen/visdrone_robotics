...
def main() -> int:
    ...
    try:
        ...
        # QOC must be exactly zero-cost at inference relative to H1 by design.
        h1_complexity = controls["H1"]["complexity"]
        if complexity.get("params") != h1_complexity.get("params"):
            raise RuntimeError(f"QOC params differ from H1: {complexity} vs {h1_complexity}")
        if abs(float(complexity.get("gflops")) - float(h1_complexity.get("gflops"))) > 1e-4:
            raise RuntimeError(f"QOC GFLOPs differ from H1: {complexity} vs {h1_complexity}")
        ...
        eval_path = report_dir / "qoc_v1_5e_eval.json"
        run_live(
        ...
        q1 = {
            "id": "Q1",
            "preset": "edge27_head_qoc_v1",
            "description": "H1 direct-reg head + training-only one-sided Quality Overconfidence Constraint",
            "status": "complete",
            "rerun": True,
        ...
        for control_id, control in controls.items():
            q1[f"delta_best_eval_vs_{control_id.lower()}"] = {
                key: float(q1["best_eval"][key]) - float(control["best_eval"][key])
                for key in ("precision", "recall", "map50", "map50_95")
            }
            q1[f"delta_complexity_vs_{control_id.lower()}"] = {
                "params": int(complexity["params"]) - int(control["complexity"]["params"]),
                "gflops": float(complexity["gflops"]) - float(control["complexity"]["gflops"]),
            }

        summary = {
            "purpose": "QOC v1 local 5e confidence-alignment screen; not final paper evidence.",
            "novelty_status": "engineering hypothesis only; distinct from replacing class targets with IoU quality or pairwise ranking, no novelty claim",
            "control_policy": "Reuse H0/H1 from prior head screen; train only Q1.",
            "mechanism": {
                ...
                "extra_train_term": "relu(class_confidence - (detached IoU + margin))^2 on assigned positives only",
                "qoc_lambda": float(cfg["presets"]["edge27_head_qoc_v1"]["qoc_lambda"]),
                "qoc_margin": float(cfg["presets"]["edge27_head_qoc_v1"]["qoc_margin"]),
            ...
            "controls": controls,
            "Q1": q1,
            "caution": "deterministic=false and 5 epochs only; promote to 50e only if Q1 beats H1 convincingly without losing the H1 efficiency point.",
        }
