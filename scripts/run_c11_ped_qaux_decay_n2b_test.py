#!/usr/bin/env python3
"""Run C11 pedestrian-only quality auxiliary supervision with cosine decay."""
...
C10_SUMMARY = ROOT / "reports/yoloedge27/stage20/c10_ped_qaux_n2b_v1_5e/summary.json"
...
def locked_schedule(epochs: int) -> list[float]:
    from src.aux_schedule import cosine_to_zero_weight

    return [cosine_to_zero_weight(BASE_AUX_WEIGHT, e, epochs) for e in range(epochs)]


def architecture_sanity(report_dir: Path):
    from src.runtime import prepare_runtime
    from scripts.train import _install_c11_aux_schedule
    ...
    assert cfg.get("c11_aux_schedule") == "cosine_to_zero"
    assert abs(float(cfg.get("c11_aux_base_weight")) - BASE_AUX_WEIGHT) <= 1e-12
    ...
    assert abs(float(criterion.aux_weight) - BASE_AUX_WEIGHT) <= 1e-9
    ...
    schedule = locked_schedule(5)
    assert abs(schedule[0] - 0.10) <= 1e-12
    assert abs(schedule[2] - 0.05) <= 1e-12
    assert abs(schedule[-1]) <= 1e-15
    assert all(a >= b for a, b in zip(schedule, schedule[1:]))

    before = len(model.callbacks.get("on_train_epoch_start", []))
    _install_c11_aux_schedule(model, cfg)
    after = len(model.callbacks.get("on_train_epoch_start", []))
    assert after == before + 1, (before, after)

    # Same C10 mechanism: pedestrian-only QAux, detached regression quality.
    model.model.train()
    ...
    text = "\n".join(
        [
            "C11 PED-ONLY P2 QUALITY-AUX COSINE-DECAY N2b SANITY PASSED",
            ...
            "aux_schedule=cosine_to_zero",
            "aux_base_weight=0.10",
            "schedule_5e=" + ",".join(f"{x:.8f}" for x in schedule),
            "final_epoch_aux_weight=0",
    ...
    return {"params": params, "gflops": gflops, "aux_schedule_5e": schedule}


def main():
    args = parse_args()
    if args.epochs != 5:
        raise ValueError("C11 screening is locked to exactly 5 epochs")
    ...
    c10 = json.loads(C10_SUMMARY.read_text(encoding="utf-8")) if C10_SUMMARY.exists() else None
    ...
    cfg["c11_aux_schedule"] = "cosine_to_zero"
    cfg["c11_aux_base_weight"] = BASE_AUX_WEIGHT
    cfg["train"]["epochs"] = 5
    ...
    schedule = locked_schedule(5)
    summary = {
        "purpose": "C11 pedestrian-only quality-aware P2 supervision with cosine auxiliary decay; 5e screen only.",
        "mechanism": {
            "name": "Decayed Pedestrian P2 Quality-Tempered Auxiliary Supervision (D-Ped-P2-QTAS)",
            "changed_scope": "training loss schedule only; inference graph unchanged",
            "focus_classes": ["pedestrian"],
            "people_auxiliary": False,
            "tiny_min_side_px": 16.0,
            "soft_target": "0.5 + 0.5 * sqrt(IoU_detached)",
            "aux_base_weight": BASE_AUX_WEIGHT,
            "aux_schedule": "cosine_to_zero across epochs",
            "aux_weights_5e": schedule,
            "final_epoch_aux_weight": 0.0,
            "main_assigner": "stock TAL exact",
            "main_loss": "stock box/cls/direct-reg1 plus decaying pedestrian-only auxiliary cls term",
            "inference_graph_change": False,
        ...
        "c10_reference": c10.get("candidate") if c10 else None,
    ...
    try:
        ...
        eval_path = report_dir / "c11_ped_qaux_decay_n2b_v1_eval.json"
        ...
        candidate = {
            "status": "complete",
            "complexity": {"params": complexity["params"], "gflops": complexity["gflops"]},
        ...
        if c10 and c10.get("candidate", {}).get("status") == "complete":
            ref = c10["candidate"]
            summary["comparison_vs_c10"] = {
                "delta_candidate_minus_c10": delta_dict(candidate["best_eval"], ref["best_eval"]),
                "delta_focus_candidate_minus_c10": {
                    cls: delta_dict(candidate["focus_best_eval"][cls], ref["focus_best_eval"][cls])
                    for cls in ("pedestrian", "people")
                },
                "delta_focus_map50_95_mean": focus_mean - float(ref["focus_map50_95_mean"]),
            }
        ...
        summary["caution"] = "single local seed, deterministic=false, 5 epochs only; C11 changes only C10 auxiliary weight schedule from constant 0.10 to cosine-to-zero."
