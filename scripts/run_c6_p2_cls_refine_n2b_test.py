#!/usr/bin/env python3
"""Run C6 P2 classification-only refinement screening on frozen N2b direct-reg1."""
...
def architecture_sanity(report_dir: Path):
    ...
    assert detect.__class__.__name__ == "P2ClsDetect"
    ...
    # Mechanism guard: changing the P2 classification refiner must not change
    # box predictions, while P2 classification scores must actually respond.
    detect.eval()
    feats = [
        torch.randn(1, 40, 16, 16),
        torch.randn(1, 64, 8, 8),
        torch.randn(1, 104, 4, 4),
    ]
    with torch.no_grad():
        refine.alpha.fill_(0.0)
        out0 = detect.forward_head(feats, **detect.one2many)
        refine.alpha.fill_(0.8)
        out1 = detect.forward_head(feats, **detect.one2many)
        refine.alpha.fill_(0.10)
    assert torch.equal(out0["boxes"], out1["boxes"]), "C6 refinement leaked into regression"
    assert not torch.allclose(out0["scores"], out1["scores"]), "C6 refinement does not affect classification"
    assert out0["feats"][0] is feats[0]
    ...
    assert gflops <= MAX_GFLOPS

    text = "\n".join(
        [
            "C6 P2-CLS-REFINE N2b SANITY PASSED",
            f"data_yaml={data_yaml}",
            f"model_yaml={model_yaml}",
            f"assigner={criterion.assigner.__class__.__name__}",
            "SPR=P4->P5",
            "neck_nominal=160/256/416",
            "head=P2ClsDetect reg_max=1",
            "regression_inputs=stock-P2/stock-P3/stock-P4",
            "classification_inputs=refined-P2/stock-P3/stock-P4",
            "p2_cls_refine=DW3x3+PW1x1 residual alpha=0.10",
            "box_isolation_guard=passed",
            f"params={params}",
            f"delta_params={params - BASE_PARAMS}",
            f"gflops={gflops:.6f}",
            f"delta_gflops={gflops - BASE_GFLOPS:.6f}",
            "inference_graph_change=true_classification_only",
        ]
    ) + "\n"
    print(text)
...
def main():
    args = parse_args()
    if args.epochs != 5:
        raise ValueError("C6 screening is locked to exactly 5 epochs")
    ...
    summary = {
        "purpose": "C6 P2 classification-only refinement on frozen N2b; 5e screening only, not final paper evidence.",
        "mechanism": {
            "name": "P2 Classification-Only Residual Refinement (P2-CR)",
            "changed_scope": "P2 classification cv3 input only",
            "regression_path": "stock P2/P3/P4 exact topology",
            "classification_path": "refined P2; stock P3/P4",
            "block": "DW3x3 + BN + SiLU + PW1x1 + BN + alpha residual",
            "alpha_init": 0.10,
            "assigner": "stock TAL exact",
            "loss": "stock",
            "reg_max": 1,
            "inference_overhead": "one lightweight stride-4 block on classification branch only",
        },
        "protocol": {
            "epochs": 5,
            "batch": args.batch,
            ...
            "seed": 42,
            "pretrained": False,
            "architecture": "frozen C1 SPR + C2 N2b + direct reg1 + P2 classification-only refine",
            "test_dev_used": False,
        },
    ...
    try:
        ...
        eval_path = report_dir / "c6_p2_cls_refine_n2b_v1_eval.json"
