...
Q1_SUMMARY = ROOT / "reports" / "yoloedge27" / "stage3" / "head_qoc_v1_5e" / "summary.json"
...
def main() -> int:
    ...
    if not Q1_SUMMARY.exists():
        raise FileNotFoundError(f"Existing Q1 result not found: {Q1_SUMMARY}")
    prior = json.loads(Q1_SUMMARY.read_text(encoding="utf-8"))
    controls = {
        "H0": prior["controls"]["H0"],
        "H1": prior["controls"]["H1"],
        "Q1": prior["Q1"],
    }
    ...
    try:
        ...
        # v2 must remain exactly on the H1/Q1 inference-complexity point.
        h1_complexity = controls["H1"]["complexity"]
        if complexity.get("params") != h1_complexity.get("params"):
            raise RuntimeError(f"QOC v2 params differ from H1: {complexity} vs {h1_complexity}")
        if abs(float(complexity.get("gflops")) - float(h1_complexity.get("gflops"))) > 1e-4:
            raise RuntimeError(f"QOC v2 GFLOPs differ from H1: {complexity} vs {h1_complexity}")
        ...
        eval_path = report_dir / "qoc_v2_5e_eval.json"
        run_live(
        ...
        q2 = {
            "id": "Q2",
            "preset": "edge27_head_qoc_v2",
            "description": "H1 direct-reg head + tiny-aware continuous-margin QOC",
            "status": "complete",
            "rerun": True,
        ...
        for control_id, control in controls.items():
            q2[f"delta_best_eval_vs_{control_id.lower()}"] = metric_delta(q2["best_eval"], control["best_eval"])
            q2[f"delta_focus_vs_{control_id.lower()}"] = focus_delta(
                q2["focus_best_eval"], control.get("focus_best_eval", {})
            )
            q2[f"delta_complexity_vs_{control_id.lower()}"] = {
                "params": int(complexity["params"]) - int(control["complexity"]["params"]),
                "gflops": float(complexity["gflops"]) - float(control["complexity"]["gflops"]),
            }

        p = cfg["presets"]["edge27_head_qoc_v2"]
        summary = {
            "purpose": "QOC v2 local 5e tiny-tolerance mechanism screen; not final paper evidence.",
            "novelty_status": "engineering hypothesis only; no novelty claim",
            "control_policy": "Reuse H0/H1/Q1; train only Q2.",
            "mechanism": {
                "base": "H1 global DFL-free direct regression (reg_max=1)",
                "assignment": "standard TaskAlignedAssigner unchanged",
                "classification": "standard TAL soft-target BCE unchanged",
                "box_loss": "standard CIoU + direct-regression unchanged",
                "extra_train_term": "relu(class_confidence - (detached IoU + adaptive_margin))^2",
                "adaptive_margin": "base + bonus*(1-clamp(min_side_px/threshold,0,1))",
                "qoc_lambda": float(p["qoc_lambda"]),
                "base_margin": float(p["qoc_margin"]),
                "tiny_threshold_px": float(p["qoc_tiny_threshold"]),
                "tiny_margin_bonus": float(p["qoc_tiny_margin_bonus"]),
                "example_margins": {"0px": 0.15, "4px": 0.125, "8px": 0.10, "16px_or_more": 0.05},
            ...
            "controls": controls,
            "Q2": q2,
            "decision_rule": "Promote only if Q2 preserves Q1 aggregate gain and materially recovers pedestrian/people without leaving H1 complexity.",
            "caution": "deterministic=false and 5 epochs only; any small delta needs later confirmation.",
        }
