#!/usr/bin/env python3
"""Run the frozen C3-v2 Tiny Center-Stable Ranking (TCSR) 5e screen.

Inference architecture is fixed to C1+C2:
  S1 SPR P4->P5 + N2b 160/256/416 neck + Detect reg_max=1.
Only TAL top-k positive ranking for tiny GTs is changed. The existing N2b 5e
run is reused as the control and is never retrained here.
"""
...
def main():
    args = parse_args()
    if args.epochs != 5:
        raise ValueError("C3-v2 screening is preregistered to exactly 5 epochs")
    ...
    if cfg.get("preset") != "edge27_c3_tcsr_v2":
        raise RuntimeError("C3-v2 runner requires preset=edge27_c3_tcsr_v2")
    ...
    summary = {
        "purpose": "C3-v2 Tiny Center-Stable Ranking local 5e screen; not final paper evidence.",
        "novelty_status": "working optimization hypothesis only; no novelty claim",
        "mechanism": {
            "name": "Tiny Center-Stable Ranking (TCSR)",
            "changed_scope": "TAL tiny-GT top-k positive ranking only",
            "candidate_region": "unchanged standard TAL",
            "ranking": "standard TAL alignment multiplied by a size-weighted anchor centerness prior",
            "conflict_resolution": "unchanged standard TAL",
            "soft_target_normalization": "unchanged standard TAL over selected positives",
            "target_boxes": "unchanged standard TAL",
            "tiny_min_side": 16.0,
            "extra_strength_hyperparameter": "none",
    ...
    try:
        ...
        if complexity["params"] != EXPECTED_PARAMS or abs(complexity["gflops"] - EXPECTED_GFLOPS) > 1e-4:
            raise RuntimeError(
                f"C3-v2 changed inference architecture: {complexity}, expected "
        ...
        eval_path = report_dir / "c3_tcsr_v2_eval.json"
