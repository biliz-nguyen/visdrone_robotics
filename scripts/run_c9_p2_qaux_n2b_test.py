#!/usr/bin/env python3
"""Run C9 quality-aware training-only P2 auxiliary supervision on frozen N2b."""
...
C8_SUMMARY = ROOT / "reports/yoloedge27/stage18/c8_p2_aux_n2b_v1_5e/summary.json"
...
def architecture_sanity(report_dir: Path):
    ...
    # Training-only guard: soft target must depend on detached box quality.
    # Auxiliary backward may update P2 classification/upstream features, but
    # it must not create gradient in box-head weights directly.
    model.model.train()
    ...
    text = "\n".join(
        [
            "C9 P2 QUALITY-AUX N2b SANITY PASSED",
...
def main():
    args = parse_args()
    if args.epochs != 5:
        raise ValueError("C9 screening is locked to exactly 5 epochs")
    ...
    c8 = json.loads(C8_SUMMARY.read_text(encoding="utf-8")) if C8_SUMMARY.exists() else None
    ...
    summary = {
        "purpose": "C9 quality-aware training-only P2 tiny-center supervision on frozen N2b; 5e screen only.",
        "mechanism": {
            "name": "P2 Quality-Tempered Auxiliary Supervision (P2-QTAS)",
        ...
        "c8_reference": c8.get("candidate") if c8 else None,
    ...
    try:
        ...
        eval_path = report_dir / "c9_p2_qaux_n2b_v1_eval.json"
        ...
        if c8 and c8.get("candidate", {}).get("status") == "complete":
            c8_candidate = c8["candidate"]
            summary["comparison_vs_c8"] = {
                "delta_candidate_minus_c8": delta_dict(candidate["best_eval"], c8_candidate["best_eval"]),
                "delta_focus_candidate_minus_c8": {
                    cls: delta_dict(candidate["focus_best_eval"][cls], c8_candidate["focus_best_eval"][cls])
                    for cls in ("pedestrian", "people")
                },
                "delta_focus_map50_95_mean": focus_mean - float(c8_candidate["focus_map50_95_mean"]),
            }
        ...
        summary["caution"] = "single local seed, deterministic=false, 5 epochs only; C9 target mapping locked before run, no grid search."
        (report_dir / "paths.txt").write_text(
            f"best_pt={best}\ncontrol_eval={CONTROL_EVAL}\nc8_summary={C8_SUMMARY}\ncheckpoint_note=runner-local only\n",
            encoding="utf-8",
        )
        (report_dir / "summary.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
