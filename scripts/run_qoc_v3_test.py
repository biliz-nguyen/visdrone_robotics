...
Q2_SUMMARY = ROOT / "reports" / "yoloedge27" / "stage3" / "head_qoc_v2_5e" / "summary.json"
...
def main() -> int:
    ...
    if not Q2_SUMMARY.exists():
        raise FileNotFoundError(f"Existing Q2 result not found: {Q2_SUMMARY}")
    prior = json.loads(Q2_SUMMARY.read_text(encoding="utf-8"))
    controls = {
        "H0": prior["controls"]["H0"],
        "H1": prior["controls"]["H1"],
        "Q1": prior["controls"]["Q1"],
        "Q2": prior["Q2"],
    }
    ...
    try:
        ...
        if complexity.get("params") != h1_complexity.get("params"):
            raise RuntimeError(f"QOC v3 params differ from H1: {complexity} vs {h1_complexity}")
        if abs(float(complexity.get("gflops")) - float(h1_complexity.get("gflops"))) > 1e-4:
            raise RuntimeError(f"QOC v3 GFLOPs differ from H1: {complexity} vs {h1_complexity}")
        ...
        eval_path = report_dir / "qoc_v3_5e_eval.json"
        ...
        q3 = {
            "id": "Q3",
            "preset": "edge27_head_qoc_v3",
            "description": "H1 direct-reg head + midpoint tiny-aware continuous-margin QOC",
            "status": "complete",
            "rerun": True,
        ...
        for control_id, control in controls.items():
            q3[f"delta_best_eval_vs_{control_id.lower()}"] = metric_delta(q3["best_eval"], control["best_eval"])
            q3[f"delta_focus_vs_{control_id.lower()}"] = focus_delta(q3["focus_best_eval"], control.get("focus_best_eval", {}))
            q3[f"delta_complexity_vs_{control_id.lower()}"] = {
                "params": int(complexity["params"]) - int(control["complexity"]["params"]),
                "gflops": float(complexity["gflops"]) - float(control["complexity"]["gflops"]),
            }

        p = cfg["presets"]["edge27_head_qoc_v3"]
        summary = {
            "purpose": "QOC v3 local 5e midpoint tiny-tolerance screen; not final paper evidence.",
            "novelty_status": "engineering hypothesis only; no novelty claim",
            "control_policy": "Reuse H0/H1/Q1/Q2; train only Q3.",
            "mechanism": {
                ...
                "example_margins": {"0px": 0.10, "4px": 0.0875, "8px": 0.075, "16px_or_more": 0.05},
            ...
            "controls": controls,
            "Q3": q3,
            "decision_rule": "Promote only if Q3 keeps Q1 aggregate gain while retaining meaningful Q2 pedestrian/people recovery at H1 complexity.",
            "caution": "deterministic=false and 5 epochs only; small deltas require later confirmation.",
        }
