#!/usr/bin/env python3
"""Run C7 P2 regression-only refinement screening on frozen N2b direct-reg1."""
...
def architecture_sanity(report_dir: Path):
    ...
    assert detect.__class__.__name__ == "P2RegDetect"
    ...
    # Mechanism guard: changing the P2 regression refiner must affect boxes
    # while classification scores stay byte-identical.
    detect.eval()
    feats = [
    ...
    assert torch.equal(out0["scores"], out1["scores"]), "C7 refinement leaked into classification"
    assert not torch.allclose(out0["boxes"], out1["boxes"]), "C7 refinement does not affect regression"
    ...
    text = "\n".join(
        [
            "C7 P2-REG-REFINE N2b SANITY PASSED",
            f"data_yaml={data_yaml}",
            f"model_yaml={model_yaml}",
            f"assigner={criterion.assigner.__class__.__name__}",
            "SPR=P4->P5",
            "neck_nominal=160/256/416",
            "head=P2RegDetect reg_max=1",
            "regression_inputs=refined-P2/stock-P3/stock-P4",
            "classification_inputs=stock-P2/stock-P3/stock-P4",
            "p2_reg_refine=DW3x3+PW1x1 residual alpha=0.10",
            "classification_isolation_guard=passed",
            f"params={params}",
            f"delta_params={params - BASE_PARAMS}",
            f"gflops={gflops:.6f}",
            f"delta_gflops={gflops - BASE_GFLOPS:.6f}",
            "inference_graph_change=true_regression_only",
        ]
    ) + "\n"
    print(text)
...
def main():
    args = parse_args()
    if args.epochs != 5:
        raise ValueError("C7 screening is locked to exactly 5 epochs")
    ...
    summary = {
        "purpose": "C7 P2 regression-only refinement on frozen N2b; 5e screening only, not final paper evidence.",
        "mechanism": {
            "name": "P2 Regression-Only Residual Refinement (P2-RR)",
            "changed_scope": "P2 regression cv2 input only",
            "regression_path": "refined P2; stock P3/P4",
            "classification_path": "stock P2/P3/P4 exact topology",
            "block": "DW3x3 + BN + SiLU + PW1x1 + BN + alpha residual",
            "alpha_init": 0.10,
            "assigner": "stock TAL exact",
            "loss": "stock",
            "reg_max": 1,
            "inference_overhead": "one lightweight stride-4 block on regression branch only",
        },
        "protocol": {
            "epochs": 5,
            "batch": args.batch,
            ...
            "seed": 42,
            "pretrained": False,
            "architecture": "frozen C1 SPR + C2 N2b + direct reg1 + P2 regression-only refine",
            "test_dev_used": False,
        },
    ...
    try:
        ...
        eval_path = report_dir / "c7_p2_reg_refine_n2b_v1_eval.json"
