...
def main():
    ...
    sprdowns_v2 = [m for m in model.model.modules() if m.__class__.__name__ == "SPRDownV2"]

    attn_names = {"ECA", "CoordAtt", "ResidualLiteCA"}
    attn_modules = [
        m for m in model.model.modules()
        if m.__class__.__name__ in attn_names
    ]

    down = cfg["backbone_down"]
    if down == "aconv":
        assert len(aconvs) == 1 and len(sprdowns) == 0 and len(sprdowns_v2) == 0
    elif down == "sprdown":
        assert len(sprdowns) == 1 and len(aconvs) == 0 and len(sprdowns_v2) == 0
    elif down == "sprdown_v2":
        assert len(sprdowns_v2) == 1 and len(aconvs) == 0 and len(sprdowns) == 0
    else:
        assert len(aconvs) == 0 and len(sprdowns) == 0 and len(sprdowns_v2) == 0
    ...
    if down in {"sprdown", "sprdown_v2"}:
        assert cfg["loss_mode"] == "standard", (
            "SPR-Down Stage-1 screening must use standard localization loss."
        )
        assert int(cfg["reg_max"]) == 16, (
            "SPR-Down Stage-1 screening must keep reg_max=16."
        )
        assert cfg["attention"] == "none", (
            "SPR-Down Stage-1 screening must not use attention."
        )
