#!/usr/bin/env python3
"""Run C4 tiny-gated NWD screening on the frozen N2b direct-reg1 model."""
...
def architecture_sanity(report_dir: Path):
    ...
    assert "blended_ciou_nwd_loss" in inspect.getsource(BboxLoss.forward)
    ...
    text = "\n".join(
        [
            "C4 TINY-NWD N2b SANITY PASSED",
            f"data_yaml={data_yaml}",
            f"model_yaml={model_yaml}",
            f"assigner={criterion.assigner.__class__.__name__}",
            "loss=tiny-gated 0.75 CIoU + 0.25 NWD below 16px",
            "non_tiny_loss=stock CIoU exact",
            "SPR=P4->P5",
...
def main():
    args = parse_args()
    if args.epochs != 5:
        raise ValueError("C4 screening is locked to exactly 5 epochs")
    ...
    cfg["c4_loss_mode"] = "tiny_nwd"
    cfg["tiny_nwd"] = {"c": 12.8, "tiny_min_side": 16.0, "ciou_weight": 0.75, "nwd_weight": 0.25}
    cfg["train"]["epochs"] = 5
    ...
    summary = {
        "purpose": "C4 tiny-gated NWD on frozen N2b; 5e screening only, not final paper evidence.",
        "mechanism": {
            "name": "Tiny-Gated NWD Localization (TGNWD)",
            "changed_scope": "training-time BboxLoss only",
            "assigner": "stock TAL exact",
            "tiny_min_side": 16.0,
            "c": 12.8,
            "ciou_weight": 0.75,
            "nwd_weight": 0.25,
            "non_tiny_behavior": "stock CIoU exact",
    ...
    try:
        ...
        eval_path = report_dir / "c4_tiny_nwd_n2b_v1_eval.json"
        ...
        summary["promotion"] = {
            ...
            "rule": "overall mAP50-95 >= N2b; mAP50 loss <=0.1pp; focus mean mAP50-95 >= N2b; each focus class no worse by >0.1pp; identical complexity; ONNX pass",
        }
