...
def test_c7_source_isolates_refinement_to_regression():
    text = (ROOT / "src" / "p2_reg_head.py").read_text(encoding="utf-8")
    assert "box_inputs = [self.p2_reg_refine(x[0]), x[1], x[2]]" in text
    assert "cls_head[i](x[i])" in text
    assert '"feats": x' in text
    assert "reg_max=1" in text


def test_model_builder_keeps_stock_n2b_feature_indices_for_c7():
    text = (ROOT / "src" / "model_builder.py").read_text(encoding="utf-8")
    assert 'c7_p2_reg_refine' in text
    assert "_detect_line(cfg, [19, 22, 25])" in text
    assert "P2RegDetect" in text
