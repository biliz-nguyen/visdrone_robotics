...
def patch_ultralytics_taq(cfg: dict) -> None:
    """Apply the normal YOLOEdge27 patch, then replace TAL with TAQ for training."""
    ...
    src = project / "src" / "tiny_quality_assigner.py"
    dst = utils_dir / "visdrone_tiny_quality_assigner.py"
    shutil.copy2(src, dst)

    text = loss_py.read_text(encoding="utf-8")
    import_line = (
        "from ultralytics.utils.visdrone_tiny_quality_assigner import "
        "TinyAdaptiveQualityAssigner"
    )
    anchor = "from ultralytics.utils.torch_utils import autocast\n"
    if import_line not in text:
        if anchor not in text:
            raise RuntimeError("Cannot find loss.py import anchor for TAQ")
    ...
    q = cfg.get("tiny_quality", {})
    tiny_min_side = float(q.get("tiny_min_side", 16.0))
    beta_floor = float(q.get("beta_floor", 4.0))
    if tiny_min_side != 16.0 or beta_floor != 4.0:
        raise ValueError("C3 TAQ-N2b screen is locked to tiny_min_side=16 and beta_floor=4")
    ...
    if old not in text:
        raise RuntimeError("Cannot find TaskAlignedAssigner constructor for TAQ patch")
