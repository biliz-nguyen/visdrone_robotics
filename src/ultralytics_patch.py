...
def patch_ultralytics(cfg: dict) -> None:
    ...
    copies = {
        ...
        project / "src" / "paired_boundary_head.py": module_dir / "visdrone_paired_boundary_head.py",
    }
...
def _patch_tasks(tasks_py: Path) -> None:
    ...
    imports = [
        ...
        "from ultralytics.nn.modules.visdrone_paired_boundary_head import PairedBoundaryDetect",
    ]
    ...
    parse_replacement = (
        "        elif m is Concat:\n"
        "            c2 = sum(ch[x] for x in f)\n"
        "        elif m is PairedBoundaryDetect:\n"
