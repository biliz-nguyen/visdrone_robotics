...
def patch_ultralytics(cfg: dict) -> None:
    ...
    copies = {
        ...
        project / "src" / "tiny_center_stable_rank.py": utils_dir / "visdrone_tiny_center_rank.py",
    }
    ...
    if assigner_mode == "tiny_center_rank":
        _patch_tiny_center_rank_assigner(loss_py, cfg)
...
def _patch_tiny_center_rank_assigner(loss_py: Path, cfg: dict) -> None:
    """Replace only the training assigner; the inference graph is untouched."""
    text = loss_py.read_text(encoding="utf-8")
    import_line = (
        "from ultralytics.utils.visdrone_tiny_center_rank import "
        "TinyCenterStableRankAssigner"
    )
    if import_line not in text:
        anchor = "from ultralytics.utils.torch_utils import autocast\n"
        if anchor not in text:
            raise RuntimeError("Cannot find loss.py import anchor for C3-v2/TCSR")
        text = text.replace(anchor, anchor + import_line + "\n", 1)

    old = _task_aligned_constructor()
    q = cfg["tiny_center_rank"]
    new = f"""        self.assigner = TinyCenterStableRankAssigner(
            topk=tal_topk,
            num_classes=self.nc,
            alpha=0.5,
            beta=6.0,
    ...
    if old not in text:
        raise RuntimeError("Cannot find TaskAlignedAssigner construction in loss.py for C3-v2/TCSR")
