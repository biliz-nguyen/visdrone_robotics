...
def patch_ultralytics(cfg: dict) -> None:
    ...
    copies = {
        ...
        project / "src" / "mentor_transfer_head.py": module_dir / "visdrone_mentor_transfer_head.py",
        project / "src" / "mentor_transfer_loss.py": utils_dir / "visdrone_mentor_transfer_loss.py",
    }
    ...
    if cfg["loss_mode"] != "standard":
        raise ValueError("Mentor-transfer branch supports standard localization loss only")
...
def _patch_tasks(tasks_py: Path) -> None:
    ...
    imports = [
        ...
        "from ultralytics.nn.modules.visdrone_mentor_transfer_head import AdvantageGatedMentorDetect",
    ]
    ...
    parse_replacement = (
        "        elif m is Concat:\n"
        "            c2 = sum(ch[x] for x in f)\n"
        "        elif m in frozenset({StrideRegDetect, AdvantageGatedMentorDetect}):\n"
    ...
    criterion_new = (
        ...
        "        if head_name == \"AdvantageGatedMentorDetect\":\n"
        "            from ultralytics.utils.visdrone_mentor_transfer_loss import AdvantageGatedTinyLocalizationLoss\n"
        "            return AdvantageGatedTinyLocalizationLoss(self)\n"
        "        return E2ELoss(self) if getattr(self, \"end2end\", False) else v8DetectionLoss(self)\n"
    )
