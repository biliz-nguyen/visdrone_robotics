"""Tiny-adaptive quality alignment for Task-Aligned Assignment.

Research prototype for YOLOEdge27 Stage 2 redesign.

The standard Task-Aligned Assigner (TAL) ranks candidates with

    classification_score ** alpha * IoU ** beta

using a fixed localization exponent ``beta`` for every ground-truth object.
For tiny boxes, a one- or two-pixel localization perturbation can change IoU
strongly, so a fixed large exponent may over-penalize otherwise useful tiny
candidates.

This prototype keeps the standard TAL candidate region and top-k selection.
It only changes the localization exponent for tiny ground truths using a
smooth size-dependent schedule that becomes exactly standard TAL at the tiny
threshold and remains unchanged for non-tiny objects.
...
class TinyAdaptiveQualityAssigner(TaskAlignedAssigner):
    ...
    def effective_beta(self, gt_bboxes: torch.Tensor) -> torch.Tensor:
        """Return per-GT localization exponent, shape ``(B, G, 1)``.

        For a ground truth with minimum side ``s``:

            r = clamp(s / tiny_min_side, 0, 1)
            beta_eff = beta_floor + (beta - beta_floor) * r

        Therefore:
        - extremely tiny GTs approach ``beta_floor``;
        - GTs at the threshold use the original TAL ``beta`` exactly;
        - non-tiny GTs are identical to standard TAL.
        """
    ...
    def get_box_metrics(
        ...
        """Compute standard TAL metrics with tiny-adaptive localization exponent."""
